...
def client(monkeypatch):
    """Create test client with multiuser mode (auth required)."""
...
# ---- multi-user mode (always required) ----

def test_me_unauthenticated_in_multiuser(client):
...
def test_deploys_api_returns_401_without_login_in_multiuser(client):
    r = client.get("/api/deploys")
    assert r.status_code == 401


def test_setup_admin_then_login_then_api(client):
    # 1. Setup first admin
    r = client.post("/api/auth/setup-admin",
...
def test_cannot_login_as_singleuser(client):
...
def test_setup_admin_requires_strong_enough_password(client):
    r = client.post("/api/auth/setup-admin",
...
def test_me_reports_needs_setup_true_when_multiuser_and_no_admin(client):
    r = client.get("/api/auth/me")
    assert r.status_code == 200
    body = r.json()
    assert body["authenticated"] is False
...
def test_me_reports_needs_setup_false_after_admin_created(client):
    ...
    r = client.get("/api/auth/me", cookies={})
    body = r.json()
...
def test_setup_admin_auto_logs_in(client):
...
def test_401_returns_structured_error_body(client):
    r = client.get("/api/deploys")
    assert r.status_code == 401
    body = r.json()
    assert body == {
